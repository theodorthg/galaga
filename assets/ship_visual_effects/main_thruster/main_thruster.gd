...
func _process(delta: float) -> void:
	if not Engine.is_editor_hint():
		...
		var angle_difference_wrapped := wrapf(_last_frame_rotation - global_rotation, -PI, PI)
		_last_frame_rotation = global_rotation
		_angle_difference_smoothed = lerp_angle(_angle_difference_smoothed, angle_difference_wrapped, 8.0 * delta)
